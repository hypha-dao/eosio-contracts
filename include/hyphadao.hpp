...
namespace hyphaspace
{
   ...
   public:
      ...
      ACTION vote(const name &voter,
                  const checksum256 &proposal_hash,
                  std::vector<document_graph::content_group> &vote_selections);
      ACTION votepass(const name &voter,
                      const checksum256 &proposal_hash);
      ACTION votefail(const name &voter,
                      const checksum256 &proposal_hash);
   ...
   private:
      ...
      void vote_option_key(const name &voter,
                           const checksum256 &proposal_hash,
      ...
      bool did_pass(const checksum256 &proposal_hash);
      name get_ballot_type (const checksum256& proposal_hash);
      void verify_membership(const name &member);
      asset get_voting_power (const name &member);
