...
trail::trail(name self, name code, datastream<const char*> ds) : contract(self, code, ds), environment(self, self.value) {
    if (!environment.exists()) {
        ...
        env_struct = env{
            self, //publisher
            new_totals, //totals
            current_block_time().to_time_point().sec_since_epoch(), //time_now
            0 //last_ballot_id
        };
        ...
        env_struct.time_now = current_block_time().to_time_point().sec_since_epoch();
    }
}
...
void trail::transfer(name sender, name recipient, asset amount) {
    ...
    if (scb == sendercb.end()) { //NOTE: sender doesn't have a counterbalance yet
        sendercb.emplace(sender, [&]( auto& a ){
            ...
            a.last_decay = current_block_time().to_time_point().sec_since_epoch();
        });
    } else {
        auto scbal = *scb;
...
void trail::mirrorcast(name voter, symbol token_symbol) {
    ...
    if (cb != counterbals.end()) { //NOTE: if no cb found, give cb of 0
        auto counter_bal = *cb;
        //check(current_block_time().to_time_point().sec_since_epoch() - counter_bal.last_decay >= MIN_LOCK_PERIOD, "cannot get more votes until min lock period is over");
...
void trail::addcandidate(name publisher, uint64_t ballot_id, name new_candidate, string info_link) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < board.begin_time , "cannot add candidates once voting has begun");
...
void trail::setallcands(name publisher, uint64_t ballot_id, vector<candidate> new_candidates) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < board.begin_time , "cannot change candidates once voting has begun");
...
void trail::setallstats(name publisher, uint64_t ballot_id, vector<uint8_t> new_cand_statuses) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() > board.end_time , "cannot change candidate statuses until voting has ended");
...
void trail::rmvcandidate(name publisher, uint64_t ballot_id, name candidate) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < board.begin_time, "cannot remove candidates once voting has begun");
...
void trail::setseats(name publisher, uint64_t ballot_id, uint8_t num_seats) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < board.begin_time, "cannot set seats after voting has begun");
...
bool trail::delete_proposal(uint64_t prop_id, name publisher) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < prop.begin_time, "cannot delete proposal once voting has begun");
...
bool trail::close_proposal(uint64_t prop_id, uint8_t pass, name publisher) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() > prop.end_time, "can't close proposal while voting is still open");
...
bool trail::delete_election(uint64_t elec_id, name publisher) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < elec.begin_time, "cannot delete election once voting has begun");
...
bool trail::delete_leaderboard(uint64_t board_id, name publisher) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() < board.begin_time, "cannot delete leaderboard once voting has begun");
...
bool trail::close_leaderboard(uint64_t board_id, uint8_t pass, name publisher) {
    ...
    check(current_block_time().to_time_point().sec_since_epoch() > board.end_time, "cannot close leaderboard while voting is still open");
...
void trail::update_from_cb(name from, asset amount) {
    ...
    if (cb_itr == fromcbs.end()) {
		uint32_t new_now = current_block_time().to_time_point().sec_since_epoch();
