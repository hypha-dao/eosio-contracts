...
void hyphadao::edit(const name &scope,
	...
	const name editor = names.at("editor");
	qualify_proposer(editor);

	check(has_auth(editor) || has_auth(get_self()), "Authentication failed. Must have authority from owner: " +
													   editor.to_string() + "@active or " + get_self().to_string() + "@active.");
	...
	o_t.emplace(get_self(), [&](auto &o) {
		...
		o.names["original_scope"] = scope;
		o.ints["original_object_id"] = id;
		o.names["ballot_id"] = register_ballot(editor, strings);
	});
}
