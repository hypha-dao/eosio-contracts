...
var devHome, daoHome, daoWasm, daoAbi, tokenHome, tokenWasm, tokenAbi, tdHome, tdWasm, tdAbi string
...
type Environment struct {
	...
	VotingDurationSeconds uint64
	HyphaDeferralFactor   uint64
	SeedsDeferralFactor   uint64
...
func SetupEnvironment(t *testing.T) *Environment {
	...
	_, err = dao.SetIntConfigAtt(env.ctx, &env.api, env.DAO, "voting_duration_sec", env.VotingDurationSeconds)
	assert.NilError(t, err)

	_, err = dao.SetIntConfigAtt(env.ctx, &env.api, env.DAO, "seeds_deferral_factor_x100", env.SeedsDeferralFactor)
	assert.NilError(t, err)

	_, err = dao.SetIntConfigAtt(env.ctx, &env.api, env.DAO, "hypha_deferral_factor_x100", env.HyphaDeferralFactor)
	assert.NilError(t, err)
	...
	dao.SetNameConfigAtt(env.ctx, &env.api, env.DAO, "treasury_contract", env.Bank)
...
func SetupMember(t *testing.T, ctx context.Context, api *eos.API,
	...
	memberAccount, err := eostest.CreateAccountFromString(ctx, api, memberName, defaultKey)
	assert.NilError(t, err)
