...
// ClosePropOld is closing proposals for document proposals
type ClosePropOld struct {
...
func Propose(ctx context.Context, api *eos.API, contract, proposer eos.AccountName, proposalType eos.Name, content string) (string, error) {
	...
	if err != nil {
		return "error", fmt.Errorf("Propose : %v", err)
	}

	dump["proposer"] = proposer
	dump["proposal_type"] = proposalType
...
// CloseProposalOld ...
func CloseProposalOld(ctx context.Context, api *eos.API, contract, closer eos.AccountName,
	proposalID uint64) (string, error) {
	...
	actions := []*eos.Action{{
		...
		ActionData: eos.NewActionData(&ClosePropOld{
